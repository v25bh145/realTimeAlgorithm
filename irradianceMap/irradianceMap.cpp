...
unsigned loadCubemap(vector<std::string> faces);
unsigned loadHDRmap(string HDRFileName);
void renderCube();
...
int main()
{
    ...
    // -------------------------
    Shader HDRToCubemap("HDRToCubemap.vert", "HDRToCubemap.frag");
    Shader convolutionShader("HDRToCubemap.vert", "convolution.frag");
    ...
    unsigned int hdrTexture = loadHDRmap("Chelsea_Stairs_3k.hdr");

    // offline rendering
    // -----------------
    
    // HDR(equirectangular) to cubemap
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render skybox to print cubemap
        cubemap.use();
