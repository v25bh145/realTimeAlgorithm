...
unsigned int loadCubemap(vector<string> paths, int& imageSize, int& nrComponents, vector<unsigned char*>& datas, bool getData) {
    unsigned cubemapTexture = 0;
    if (paths.size() < 6) {
        cout << "Failed to load cubemap: paths vector size() < 6" << endl;
    }
    else {
...
void createTextureCubeMapNull(unsigned& texture, unsigned width, unsigned height, unsigned nrComponent) {
    ...
    for (unsigned i = 0; i < 6; ++i) {
        ...
        case GL_RGB:
            ...
            break;
        default:
            cout << "texture error at createTextureCubeMapNull: unknown nrComponent" << endl;
            return;
        }
    }
...
void createTexture2DNull(unsigned& texture, unsigned width, unsigned height, unsigned internalFormat, unsigned format, unsigned type) {
    ...
    cout << "GL_TEXTURE_2DNull: " << glGetError() << endl;
...
void createTexture3DNull(unsigned& texture, unsigned width, unsigned height, unsigned depth, unsigned internalFormat, unsigned format, unsigned type) {
    ...
    cout << "createTexture3DNull: " << glGetError() << endl;
